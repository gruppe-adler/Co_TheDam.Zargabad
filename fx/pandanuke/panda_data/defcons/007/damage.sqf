...
if ( schaden_an ) then
{
  _vehicles = argt_vehicles;
  _objects = argt_objects;
} else
{
  _vehicles = nearestobjects [[_xpos, _ypos , 0], ["Car", "Motorcycle", "Tank", "Air", "Ship"], strahlungs_radius * 1.3];
...
[_xpos, _ypos] execvm "\panda_syndicate\panda_data\defcons\007\damage2.sqf";
...
_airs = main_airs;  };
{
  ...
  {[_x] execvm "\panda_syndicate\panda_data\defcons\007\atomemp_s7.sqf"} foreach _vehicles;
  ...
  else
  {
    if ( _x iskindof "Man" || _x iskindof "Car" || _x iskindof "Motorcycle" || _x iskindof "Tank" || _x iskindof "Ship" ) then
    {
      ...
      if ( _x iskindof "Man" ) then {_speed = _speed * 2}
        else {[_x, _speed * 2] execvm "\panda_syndicate\panda_data\defcons\007\rotate.sqf"};
      _speed = _speed * Druck_welle_spd;
      _x setvelocity [(_vel select 0) + (_speed * cos _dir),
                      (_vel select 1) + (_speed * sin _dir),
                      _speed / 3];
