...
_soundPos = "Land_HelipadEmpty_F" createVehiclelocal (position _pos);
if ((isPlayer _unit or player in _unit)and(_unit distance _pos < ((ln(_kT)+1) * 4500))) then
{

	_soundPos = "Land_HelipadEmpty_F" createVehicle (position _pos);  //Land_HelipadEmpty_F
};

// -- Cycle which waits for a heatWave comes to a player
while {panda_hitzewelledistanz < _rad and panda_hitzewelledistanz < panda_hitzewelleausbreitung} do
{
	_rad = _unit distance _pos;  //fred
	...
	_xSpos = panda_schockausbreitung * cos (_fi); 
	_ySpos = panda_schockausbreitung * sin (_fi);
	_soundPos setPos [((getPos _pos select 0)+_xSPos),((getPos _pos select 1)+_ySPos),0];
	sleep 0.1;
};
...
if (vehicle _unit == _unit and !_shockWavePassed) then
{
	...
	if (!(_x in _unit)) then
	{
		...
		{
			[_x,_pos,_kT,false] spawn panda_fnc_druckwelle;};
			AllArray = Allarray + [_x];
		};
	} forEach _crewInside;

	// Sound of HeatWave impact to a unit or vehicle
	if ((isPlayer _unit or player in _unit)and(_unit distance _pos < ((ln(_kT)+1) * 4500))and(panda_hitzewelledistanz < panda_hitzewelleausbreitung)) then
		{
		...
		if (player distance _pos < ((ln(_kT)+1) * 4500)) then { playsound _Sound;};
		};

	// -- Damage applied by a HeatWave to a unit or vehicles body and its weak parts
	if (panda_hitzewelledistanz < panda_hitzewelleausbreitung) then
		{
...
while {panda_schockausbreitung < _rad and panda_schockausbreitung < panda_schockdistanz} do {
_rad = _unit distance _pos;
_xPos = ((position _unit select 0) - (position _pos select 0));
_yPos = ((position _unit select 1) - (position _pos select 1));
if (_xPos >= 0 and _yPos >= 0) then {_fi = 0  + atan (abs(_yPos / _xPos));};
...
if (_unit distance _soundPos < ln(8000 - panda_schockausbreitung)*21) then {
if (player distance _pos < ((ln(_kT)+1) * 800)) then {_intenzity = "Close";};
if (player distance _pos >= ((ln(_kT)+1) * 800) and player distance _pos < ((ln(_kT)+1) * 2000)) then {_intenzity = "Standard";};
if (player distance _pos >= ((ln(_kT)+1) * 2000) and player distance _pos < ((ln(_kT)+1) * 4500)) then {_intenzity = "Far";};
_SoundWave = "shockwave"+_intenzity;
if (player distance _pos < ((ln(_kT)+1) * 4500)) then { playsound _SoundWave; playSound "snd_impdeep3";};
};
sleep 0.2;
};
if (vehicle _unit == _unit and !_shockWavePassed) then
...
if ((isPlayer _unit or player in _unit)and(_unit distance _pos < ((ln(_kT)+1) * 4500))and(panda_schockausbreitung < panda_schockdistanz)) then {
if (player distance _pos < ((ln(_kT)+1) * 800)) then {_intenzity = "Close";};
if (player distance _pos >= ((ln(_kT)+1) * 800) and player distance _pos < ((ln(_kT)+1) * 2000)) then {_intenzity = "Standard";};
if (player distance _pos >= ((ln(_kT)+1) * 2000) and player distance _pos < ((ln(_kT)+1) * 4500)) then {_intenzity = "Far";};
...
[_x,0,0,0,0] call panda_fnc_schutzgebaude;
			...
			//hint format ["nObj: %3\nIns: %1\nBehind: %2\nPrior: %4",_insider,_behinder,_nObject,_priorityM];			
				...
				if (
					...
					_unit distance _pos > _nObject distance _pos
						...
						_acc = _acc/(36/_kT + ceil random (24/_kT));
			...
			if (_luckyDay > _mortality and _unit distance _nObject <= 15) then 
				{
				_acc = _acc/((50/_kT)+ ceil random (50/_kT));
				};

			if (_luckyDay > _mortality) then
				{
				_dam = (_dam / (1 + 10/_kT + ceil random (10/_kT)));
				_acc = _acc/((22/_kT + ceil random (9/_kT)));	
				};
			};
...
while {damage _unit < 1  or timeAfter < radTime} do
	...
	if (!(_x in _unit)) then
		...
		if (!(_x in AllArray)) then
			{
			[_x,_pos,_kT,true] spawn panda_fnc_druckwelle;};
			AllArray = Allarray + [_x];
		};
	} forEach _crewInside;
	...
	if (_fi >= _lowerAngle and _fi <= _upperAngle and count CLOUD > 0 and fallTime > 0) then
		{
		{
		_xP = ((position _unit select 0) - (_x select 0));
		_yP = ((position _unit select 1) - (_x select 1));
